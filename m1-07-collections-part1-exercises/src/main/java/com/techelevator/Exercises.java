...
public class Exercises {
	...
	public String[] list2Array(List<String> stringList) {
		...
		//String array use get to add
		for(int i = 0; i<stringList.size();i++) {
			myAnswerArray[i] = stringList.get(i);
		}
	...
	public List<String> no4LetterWords(String[] stringArray) {
		...
		for(int i=0; i<stringArray.length;i++) {
			if(stringArray[i].length()!=4) {
				myStringList.add(stringArray[i]);
			}
		}
	...
	public List<String> reverseList(List<String> stringList) {
		// how to convert to different list types 
		List<String> revList = new ArrayList<String>();
		
		for(int i = stringList.size()-1 ; i>=0; i--) {
			
			revList.add(stringList.get(i));
			}
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		List<Double> normal = new ArrayList<Double>(intArray.length);
		
		for(int i=0; i<intArray.length; i++) {
			normal.add(((double) intArray[i]/2));
	...
	public Integer findLargest(List<Integer> integerList) {
		...
		for(int i=0; i< integerList.size(); i++){
			newInt.add(integerList.size());
		}	
		Integer i = Collections.max(newInt);
		return i;
	}
	...
	public boolean foundIntTwice(List<Integer> integerList, int intToFind) {
		...
		boolean found1 = false;			//integerlist needs to use .get(i)
										//[] for arrays arrays use size
		boolean found2 = false;
		
		for(int i=0;i<integerList.size();i++) {
			if(found1==false && integerList.get(i) == intToFind) {
				found1 = true;
			}
			else if(found2==false && integerList.get(i) == intToFind) {
				found2=true;
			}
		}
    ...
    ** INTERVIEW QUESTION **	 
	...
	List<String> theBuzz = new ArrayList<String> (); 
		
		
		for(int i=0; i<integerArray.length;i++) {
			if(integerArray[i]%3==0 && integerArray[i]%5==0) {
				theBuzz.add("FizzBuzz");
			}
			...
			else theBuzz.add(integerArray[i].toString());
		}
		 return theBuzz;
	}
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		List<Integer> innerDim = new ArrayList<Integer>();
		
		for(int i=0; i <listOne.size(); i++) {
			innerDim.add(listOne.size());
		}
		for(int i = 0; i< listTwo.size(); i++) {
			innerDim.add(listTwo.size());
		}
		return innerDim;
	}
